...
test.describe('Permission System', () => {
  ...
  test('creator should have control permission, non-creator should not', async ({ createUser }) => {
    ...
    await creator.page.waitForSelector('button[title="AddVideo"]', { timeout: ROOM_LOAD_TIMEOUT });

    // Create viewer
    const viewer = await createUser('Viewer');
    await viewer.page.goto(roomUrl);
    await viewer.page.waitForSelector('button[title="AddVideo"]', { timeout: ROOM_LOAD_TIMEOUT });

    // Creator searches for video
    await creator.page.click('button[title="AddVideo"]');
