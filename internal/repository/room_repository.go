...
func (r *RoomRepository) List(ctx context.Context) ([]*model.Room, error) {
	rows, err := r.db.QueryContext(ctx,
		"SELECT id, room_id, name, creator_id, creator_name, is_active, created_at, updated_at FROM rooms WHERE is_active = TRUE ORDER BY created_at DESC LIMIT 50")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		room := &model.Room{}
		if err := rows.Scan(&room.ID, &room.RoomID, &room.Name, &room.CreatorID, &room.CreatorName, &room.IsActive, &room.CreatedAt, &room.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *RoomRepository) GetByRoomID(ctx context.Context, roomID string) (*model.Room, error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT id, room_id, name, creator_id, creator_name, is_active, created_at, updated_at FROM rooms WHERE room_id = ?",
		roomID).Scan(&room.ID, &room.RoomID, &room.Name, &room.CreatorID, &room.CreatorName, &room.IsActive, &room.CreatedAt, &room.UpdatedAt)
